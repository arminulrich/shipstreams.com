<?php
return [
    'listing.pending_save' =>
        '<span class="glyphicon glyphicon-info-sign"></span> You must save before you can add <b title=":description" class="js-tooltip">:model</b>.',
    'listing.new' => 'New',
    'actions.save_new' => 'Go to create form after saving',
    'actions.save_back' => 'Return to listing after saving',
    'actions.save' => 'Save',
    'actions.new' => '&amp; New',
    'actions.back' => '&amp; Back',
    'actions.delete' => 'Delete',
    'actions.cancel' => 'Cancel',
    'secondary_actions.duplicate' => 'Duplicate',
    'secondary_actions.changes' => '<b>Changes</b> history',
    'secondary_actions.public_view' => 'Public view',
    'belongs_to.search' => 'Search',
    'date.placeholder' => 'MM/DD/YYYY',
    'date.format' => 'm/d/Y',
    'image.crop_tooltip' =>
        'Click and drag on the image to set cropping bounds',
    'image.focal_tooltip' =>
        'Click a point that should stay in view despite browser resizing',
    'image.title_placeholder' => 'Enter image title',
    'image.title_help' => 'Used for the img tag &quot;alt&quot; attribute'
];
